...
-------------------------------------------------------------------------------
    ...
    Modified code Copyright (C) 2025 Dezhi Dai, ANL
-------------------------------------------------------------------------------
License
...
namespace Foam
{
    ...
    addToRunTimeSelectionTable(dynamicFvMesh, dynamicRefine2DFvMesh, doInit);
}

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::dynamicRefine2DFvMesh::calculateProtectedCells
(
...
void Foam::dynamicRefine2DFvMesh::readDict()
{
...
void Foam::dynamicRefine2DFvMesh::mapFields(const mapPolyMesh& mpm)
{
...
Foam::dynamicRefine2DFvMesh::refine
(
...
Foam::scalarField
Foam::dynamicRefine2DFvMesh::maxPointField(const scalarField& pFld) const
{
...
Foam::scalarField
Foam::dynamicRefine2DFvMesh::maxCellField(const volScalarField& vFld) const
{
...
Foam::scalarField
Foam::dynamicRefine2DFvMesh::cellToPoint(const scalarField& vFld) const
{
...
Foam::scalarField Foam::dynamicRefine2DFvMesh::error
(
...
void Foam::dynamicRefine2DFvMesh::selectRefineCandidates
(
...
Foam::labelList Foam::dynamicRefine2DFvMesh::selectRefineCells
(
...
{
    // Every refined cell causes 7 extra cells in 3D
    // Luca Cornolti: Every refined cell causes 3 extra cells in 2D
...
Foam::labelList Foam::dynamicRefine2DFvMesh::selectUnrefineEdges
(
...
) const
{
    ...
    forAll(splitEdges, j)
    {
        ...
        forAll(e, i)
        {
            ...
            // LC: why compare unrefineLevel with the field value
            // instead of the cell level as the name suggests?
...
void Foam::dynamicRefine2DFvMesh::extendMarkedCells
(
    bitSet& markedCell
) const
{
...
void Foam::dynamicRefine2DFvMesh::checkMaxAnchorPoints
(
    ...
    forAll(pointLevel, pointi)
    {
        ...
        for (const label celli : pCells)
        {
            if (pointLevel[pointi] <= cellLevel[celli])
            {
                ...
                if (cellPoints(celli).size() == 8 &&  nAnchorPoints[celli] == 8)
                {
                    protectedCell.set(celli);
                }

                // Luca Cornolti: Check if wedge cell has already 6 anchor points -> protect cell
                if (cellPoints(celli).size() == 6 && nAnchorPoints[celli] == 6)
                {
                    protectedCell.set(celli);
                }
...
Foam::dynamicRefine2DFvMesh::dynamicRefine2DFvMesh
(
    const IOobject& io,
    const bool doInit
)
...
bool Foam::dynamicRefine2DFvMesh::init(const bool doInit)
{
    if (doInit)
    {
...

    {
        ...
        // Luca Cornolti: Check cells for corner points
        checkMaxAnchorPoints(protectedCell_);
    }
...
bool Foam::dynamicRefine2DFvMesh::updateTopology()
{
    ...
    if (time().timeIndex() > 0 && time().timeIndex() % refineInterval == 0)
    {
        ...
        // Luca Cornolti
        const label nBufferLayersR = refineDict.lookupOrDefault<label>
        (
            "nBufferLayersR",
            1
        );
...
bool Foam::dynamicRefine2DFvMesh::update()
{
...
bool Foam::dynamicRefine2DFvMesh::writeObject
(
