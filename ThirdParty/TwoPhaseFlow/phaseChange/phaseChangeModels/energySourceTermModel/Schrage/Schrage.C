...
Foam::TwoPhaseFlow::Schrage::Schrage
(
...
Foam::tmp<Foam::fvScalarMatrix> Foam::TwoPhaseFlow::Schrage::TSource1()
{
    ...
    TSource.ref() *= mag(reconstructor_.interfaceS().internalField())/mesh.V();
...
Foam::tmp<Foam::fvScalarMatrix> Foam::TwoPhaseFlow::Schrage::TSource2()
{
    reconstructor_.reconstruct(false);
    const fvMesh& mesh = phase1_.mesh();
...
Foam::tmp<Foam::volScalarField> Foam::TwoPhaseFlow::Schrage::energySource()
{
    reconstructor_.reconstruct(false);
    const volScalarField& TSat = satModel_.TSat();
    ...
    energySourceRef.ref() *= mag(reconstructor_.interfaceS().internalField())/TSat.mesh().V();
...
Foam::tmp<Foam::volScalarField> Foam::TwoPhaseFlow::Schrage::energyFlux1()
{
    const volScalarField& TSat = satModel_.TSat();
    ...
    for (const label celli: reconstructor_.mixedCells())
    {
        interface[celli] = 1;
    }
...
Foam::tmp<Foam::volScalarField> Foam::TwoPhaseFlow::Schrage::energyFlux2()
{
    const volScalarField& TSat = satModel_.TSat();
