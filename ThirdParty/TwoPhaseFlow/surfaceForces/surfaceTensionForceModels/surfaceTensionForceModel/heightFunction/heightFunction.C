...
#include "reconstruction.H"     // Change to use SimPLIC method
...
void Foam::TwoPhaseFlow::heightFunction::getStencilValues
(
...
bool Foam::TwoPhaseFlow::heightFunction::fullColumn(const scalar avgHeight,const scalar tol)
{
    return (avgHeight >= (1-tol) || avgHeight < tol);
}

void Foam::TwoPhaseFlow::heightFunction::nextCell
(
    const label celli,
...
void Foam::TwoPhaseFlow::heightFunction::computeColumns
(
    const label dir,
    const Map<scalar>& mapAlpha,
    const label celli,
...
void Foam::TwoPhaseFlow::heightFunction::exchangeStencils
(
...
Foam::scalar Foam::TwoPhaseFlow::heightFunction::calcCurvature
(
...
Foam::TwoPhaseFlow::heightFunction::heightFunction
(
...
:
    ...
    mesh_(dynamic_cast<const Foam::fvMesh&>(alpha1.mesh())),
    stencilHF_(dynamic_cast<const Foam::fvMesh&>(alpha1.mesh()), dict.lookupOrDefault<scalar>("angleTol",0.001)),
    IFRegion_(dynamic_cast<const Foam::fvMesh&>(alpha1.mesh())),
    twoDim_(false)
...
void Foam::TwoPhaseFlow::heightFunction::correctContactAngle
(
...
)
{
    ...
    const Foam::fvMesh& mesh = dynamic_cast<const Foam::fvMesh&>(alpha1_.mesh());
...
void Foam::TwoPhaseFlow::heightFunction::correct()
{
    ...
    geometricVofExt::SimPLIC::reconstruction& reconstructor =
        mesh.lookupObjectRef<geometricVofExt::SimPLIC::reconstruction>("reconstruction");

    // can also be an isosurface
    reconstructor.reconstruct(false);

    const boolList& interfaceCells = reconstructor.interfaceCell();
    const volVectorField& faceCentre = reconstructor.interfaceC();
    const volVectorField& faceNormal = reconstructor.interfaceS();
    const globalIndex& globalIdx = zoneDistribute::New(mesh_).globalNumbering();
