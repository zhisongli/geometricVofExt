...
Foam::scalar Foam::TwoPhaseFlow::fitParaboloid::calcCurvature
(
...
Foam::vectorField Foam::TwoPhaseFlow::fitParaboloid::getFaceCentres
(
...
Foam::TwoPhaseFlow::fitParaboloid::fitParaboloid
(
...
void Foam::TwoPhaseFlow::fitParaboloid::correctContactAngle
(
...
)
{
    ...
    geometricVofExt::SimPLIC::cutFace faceCutter(mesh, reconstructor.faceFlatness());
    ...
    forAll(boundary, patchi)
    {
        if (isA<alphaContactAngleTwoPhaseFvPatchScalarField>(abf[patchi]))
        {
            ...
            forAll(nbf[patchi],i)
            {
                ...
                {
                    n /= mag(n);
                    label cutStatus = faceCutter.calcSubFace
                    (
                        faceI,
                        n,
                        centre[celli]&n
                    ...
                    if (cutStatus == 0)
                    {
                        const point cutEdgeCentre =
                             average(faceCutter.interfacePoints());
...
void Foam::TwoPhaseFlow::fitParaboloid::correct()
{
    ...
    volVectorField faceCentre = reconstructor.interfaceC();

    volVectorField faceNormal = reconstructor.interfaceS();
