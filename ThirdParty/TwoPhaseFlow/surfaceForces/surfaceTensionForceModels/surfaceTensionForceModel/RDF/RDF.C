...
:
    ...
    RDF_(reconstructedDistanceFunction(alpha1.mesh()))
{
...
template <typename T>
void Foam::TwoPhaseFlow::RDF::distributeField(const labelList& neiProcs,Field<T>& field)
{
...
Foam::labelList Foam::TwoPhaseFlow::RDF::getNeibourProcs(const boolList& nextToInterface)
{
...
Foam::label Foam::TwoPhaseFlow::RDF::closestDistToSurface(const point& p)
{
...
Foam::scalar Foam::TwoPhaseFlow::RDF::distanceToSurfacePlane(const point& p)
{
    const label& minLabel = findMin(mag(FaceCentreField_-p)());
...
void Foam::TwoPhaseFlow::RDF::correctContactAngle
(
...
void Foam::TwoPhaseFlow::RDF::correct()
{
